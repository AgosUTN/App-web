...
- Manejo adecuado de concurrencia en los dos puntos donde se pueden originar condiciones de carrera:
    - En el alta manual de un ejemplar, podría darse una condición de carrera debido al manejo en memoria del "idEjemplar" que constituye junto con "idLibro" la CP compuesta de la entidad débil "Ejemplar". Gracias a que el atributo problemático forma parte de la CP, no se necesita bloquear el registro del libro asociado, ya que si se intentase dar de alta dos ejemplares del mismo libro en paralelo, se daría un error de clave primaria duplicada. Se considera manejo de concurencia porque se tiene en cuenta dicho error y se maneja con un 409 y no con un error genérico de status 500.
