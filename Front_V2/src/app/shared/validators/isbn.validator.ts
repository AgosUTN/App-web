...
export function isbnValidator(): ValidatorFn {
  return (control: AbstractControl): ValidationErrors | null => {
    ...
    const isValidISBN10CheckDigit = isValidISBN10 && checkIsbn10Checksum(normalizedValue);

    // Comprobar la validez de los dígitos del ISBN-13
    const isValidISBN13CheckDigit = isValidISBN13 && checkIsbn13Checksum(normalizedValue);

    return isValidISBN10CheckDigit || isValidISBN13CheckDigit ? null : { invalidIsbn: true };
  };
}
